...
class DataFCDatabase:
    ...
    def _find_read_session_structure(self, path):
        ...
        self.dfSessions['session'] = ['_'.join([row['mousename'], row['dateKey'], row['sessionKey']]) for idx, row in self.dfSessions.iterrows()]
    ...
    def _find_parse_neuro_files(self, path):
        ...
        for mousename in mice:
            with h5py.File(self.datapaths[mousename], 'r') as f:
                self.sessions[mousename] = list(f['metadata'].keys())
    ...
    def get_trial_idxs(self, session, mousename, trialType=None):
        metadata = self.get_metadata(session, mousename)
        if trialType is None:
            return metadata.index
        else:
            return metadata[metadata['trialType'] == trialType].index
    ...
    def get_neuro_data(self, selector, datatype='raw', zscoreDim=None, intervName=None, trialType=None, performance=None):
        ...
        for session in sessions:
            ...
            if performance is not None:
                raise ValueError("Currently pointless to use performance")
            ...
            dataRSP = dataRSP[self.get_trial_idxs(session, mousename, trialType=trialType)]
