...
h5outname = 'gallerosalas_result_multiregional_pid_df_rand.h5'
...
for mousename in ['mou_5']:  #dataDB.mice:
    ...
    for session in dataDB.get_sessions(mousename):
        for trialType in [None, 'Hit', 'CR']:
            dataLabel = '_'.join(['PID', mousename, session, str(trialType)])
            print(dataLabel)
            if type_of_path(h5outname, dataLabel) is not None:
                print(dataLabel, 'already calculated, skipping')
            else:
                dataLst = dataDB.get_neuro_data({'session': session}, trialType=trialType)

                dataRSP = np.concatenate(dataLst, axis=0)  # Concatenate trials and sessions
                dataRSPRand = np.random.uniform(0, 1, dataRSP.shape)

                # No permutation testing, same random shape as original
                rezDF = pid.pid([dataRSPRand], mc, channelNames, labelsSrc=None, labelsTrg=None, nPerm=0, nBin=4)
